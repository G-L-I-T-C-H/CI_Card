...
import { faPeopleGroup, faCalendarXmark,faIndianRupeeSign, faCircleInfo, faPhone, faLocationDot, faTrophy ,faPerson } from '@fortawesome/free-solid-svg-icons'
import { useState } from 'react';
import { Link } from "react-router-dom";

function Modal({isVisible,onClose,name,size,deadline,fees,overview,organiser,number,src}){
    ...
    function getContent(activeTab){
        switch (activeTab){
            ...
            case "contacts":
                return (
                    <div className='p-2'>
                        <h1 className='font-semibold mb-3 md:text-xl'>Contact the organisers</h1>
    ...
    return (
        <div className='fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center border rounded-md' onClick={handleClose} id="wrapper"> 
            <div className='w-[400px] flex flex-col md:w-[80%] h-[50%]'>
                <button className='text-white text-xl place-self-end md:text-2xl' onClick={()=>onClose()}>X</button>
                <div className='bg-white p-2 rounded flex flex-col '>
                    ...
                    <div className="flex justify-start items-center space-x-1">
                            <FontAwesomeIcon icon={faCircleInfo} size={activeTab === "overview" ? "xl" : "lg"} onClick={()=>tabHandler("overview")}/>
                        </div>

                        <div className="flex justify-end items-center space-x-1">
                            <FontAwesomeIcon icon={faTrophy} size={activeTab === "prize" ? "xl" : "lg"} onClick={()=>tabHandler("prize")}/>

                        </div>

                        <div className="flex font-bold items-center space-x-1">
                            <FontAwesomeIcon icon={faLocationDot} size={activeTab === "location" ? "xl" : "lg"} onClick={()=>tabHandler("location")}/>

                        </div>

                        <div className="flex font-bold items-center space-x-1">
                            <FontAwesomeIcon icon={faPhone} size={activeTab === "contacts" ? "xl" : "lg"} onClick={()=>tabHandler("contacts")}/>
                        </div>
                    </div>
